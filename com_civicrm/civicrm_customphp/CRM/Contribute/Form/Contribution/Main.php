...
require_once 'CRM/Contribute/Form/Contribution/GiftMember.php';
require_once 'CRM/Contribute/Form/Contribution/GiftSubscription.php';
...
class CRM_Contribute_Form_Contribution_Main extends CRM_Contribute_Form_ContributionBase 
{
    ...
    {  
        ...
        $memType = 0;
        $membership = CRM_Member_BAO_Membership::getContactMembership(  $this->_userID , null, 0 ) ;
        $session = CRM_Core_Session::singleton( );
     
        if ($membership) {
             $session->set('memType', $membership[membership_type_id]);
             $memType = $membership[membership_type_id];
        }
...
// ------------  start of new block
        ...
        {
           CRM_Contribute_Form_Contribution_GiftMember::preProcess( $this );
        }    
        else
        {
            if ($title == 'Give a Gift Subscription to the Trinity Forum Society')
            {
           	      CRM_Contribute_Form_Contribution_GiftSubscription::preProcess( $this );
            }      
           else
 ...
 // ---------- end of new block           
         if ( CRM_Utils_Array::value( 'hidden_onbehalf_profile', $_POST ) ) {
           if ($title == 'Give a Gift Membership to the Trinity Forum Society') {
                CRM_Contribute_Form_Contribution_GiftMember::buildQuickForm( $this );
                }
            else
            {   
                if ($title == 'Give a Gift Subscription to the Trinity Forum Society')
       		 {
     		      CRM_Contribute_Form_Contribution_GiftSubscription::buildQuickForm( $this );
      		  }   
      		 else  
  		 {
        ...
        if (  CRM_Utils_Array::value( 'id', $this->_pcpInfo )  && 
            ...
            $intro  =  $this->_pcpInfo['intro_text'];
        } else if ( CRM_Utils_Array::value( 'intro_text', $this->_values ) ) {
            $this->assign( 'intro_text' , $this->_values['intro_text'] );
            $intro = $this->_values['intro_text'] ;
        }
        if ($title == 'Give a Gift Subscription to the Trinity Forum Society')
        {
       		$session = CRM_Core_Session::singleton( );
    		$memType = $session->get( 'memType' );
    		if ($memType == 4)
       			$intro = $intro."As a Society Memeber you recieve 1 Gift Subscripition";
       		if ($memType == 2)
       			$intro = $intro."As a Society Memeber you recieve 3 Gift Subscripitions";
       		if ($memType == 5)
       			$intro = $intro."As a Society Memeber you recieve 5 Gift Subscripitions";
       		if ($memType == 3)
       			$intro = $intro."As a Society Memeber you recieve 10 Gift Subscripitions";
       			
                $this->assign( 'intro_text' , $intro);
       }
...
//         $this->_defaults['account_holder'] = 'Max M端ller';
