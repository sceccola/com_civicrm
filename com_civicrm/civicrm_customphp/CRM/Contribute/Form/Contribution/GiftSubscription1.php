...
class CRM_Contribute_Form_Contribution_GiftSubscription1
{
    /** 
    ...
    static function preProcess( &$form )
    {
        ...
        $form->_profileId = CRM_Core_DAO::getFieldValue( 'CRM_Core_DAO_UFGroup', 'gift_member',
                                                         'id', 'name' );
    ...
    static function buildQuickForm( &$form ) 
    {
        ...
        $cnt = 1;
        $total = 2;
        $combined = $profileFields;
        echo "Size of Combined " . sizeof($combined);
        while ($cnt < $total)
        {  
        	foreach ($profileFields as $key => $value)
		{
			echo "Key ". $key . " Value " . $value  . " Cnt ".$cnt ."<br>";
			$key = $key.$cnt;
			echo "new Key " . $key . "<br>";
			$combined[$key] = $value;
        ...
        $profileFields = $combined;
           echo "Size of Profile " . sizeof($profileFields). "<br>";
  		foreach ($profileFields as $key => $value)
		{
        ...
        foreach ( $profileFields as $name => $field ) {
            if ( in_array( $field['field_type'], $fieldTypes ) ) {
                ...
                echo "<br>";
                echo "Build ".$field['name'];
                echo "Build ".$field['title'];
                if ($field['name'] == "first_name")
                	$cnt = $cnt + 1;
                $field['name'] =  $field['name'].$cnt;
                echo "Build ".$field['name'];
