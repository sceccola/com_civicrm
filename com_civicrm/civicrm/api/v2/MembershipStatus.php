...
function civicrm_membership_status_calc( $membershipParams, $excludeIsAdmin = false )
{
    ...
    $query = "
SELECT start_date, end_date, join_date, contact_id
...
";
    ...
    if ( $dao->fetch( ) ) {
        ...
        $result =&
           ...
           echo $result['name'];     
           echo " ";                                                        
           if ( $result['name'] == 'Lapsed') // check to see if record should be expired
          {  
          		 echo " See if Active ";
                 $queryLapsed = "
					SELECT contact_id FROM civicrm_membership
 							WHERE  (status_id = 9 or status_id = 11) and contact_id = " . $dao->contact_id;
 				 echo  $membershipID; 
          		 echo " ";
          		 echo $queryLapsed;     
           			echo " ";                                                        
       			
  					 $paramsLapsed = array( 1 => array( $membershipID, 'Integer' ) );
  					 $daoLapsed =& CRM_Core_DAO::executeQuery( $queryLapsed, $paramsLapsed );
   					 if ( $daoLapsed->fetch( ) ) {
   					 	 $result['id'] = 4;
                         $result['name'] = 'Expired';
                         echo  "Changed ";                                                             
